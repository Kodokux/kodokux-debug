(function(window, document, version, callback) {
    ...
    var djdt = {
        init: function() {
                ...
                if ($(this).val() !== '0') {
                    ...
                    $.getJSON(BASE_URL + 'debug/index/translateGetNamespace', {
                        ...
                        $.each(data, function(index, obj) {
                            $('#traducao-namespace').append('<option value="' + obj + '">' + obj + '</option>');
            ...
            $('#traducao-namespace').change(function() {
                if ($(this).val() !== '0') {
                    $('#debug-panel-Logs').append('<div class="loader"></div>');
                    ...
                    $.getJSON(BASE_URL + 'debug/index/translateGetModules', {
                        ...
                        $('#traducao-modulo').empty().append('<option value="0">Selecione...</option>');
                        $.each(data, function(index, obj) {
                            $('#traducao-modulo').append('<option value="' + obj + '">' + obj + '</option>');
                            $('#debug-panel-Logs .loader').remove();
            ...
            $('#traducao-executar').click(function(e) {
                e.preventDefault();
                var t = $(this);
